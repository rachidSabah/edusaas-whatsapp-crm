// Authentication utilities for Multi-Tenant WhatsApp CRM SaaS (Edge-compatible)
...
// Get JWT secret as Uint8Array for jose (Edge-compatible)
function getSecretKey(): Uint8Array {
  return new TextEncoder().encode(SESSION_CONFIG.jwtSecret);
}
...
/**
 * Generate a JWT token using jose (Edge-compatible)
 */
...
export async function authenticateUser(email: string, password: string): Promise<{ user: AuthUser; token: string } | null> {
  const result = await db.execute({
    sql: `SELECT id, email, password, name, role, organizationId, avatar, isActive FROM users WHERE email = ?`,
